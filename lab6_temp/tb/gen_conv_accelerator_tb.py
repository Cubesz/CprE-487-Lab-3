            ...
            for fc in range(FC):
                for fh in range(FH):
                    for fw in range(FW):
                        ...
                        input_val = int(input[fc][fh+oh][fw+ow])
                        filter0_val = int(filter[0][fc][fh][fw])
                        filter1_val = int(filter[1][fc][fh][fw])
                        filter2_val = int(filter[2][fc][fh][fw])
                        filter3_val = int(filter[3][fc][fh][fw])
...
sim_biases = np.fromfile("biases_8q.bin", dtype=np.int16)
sim_inputs = rng.uniform(low=-2, high=127, size=(32, 10, 10)).astype(np.int8)
sim_filters = rng.uniform(low=-127, high=128, size=(32, 32, 5, 5)).astype(np.int8)
sim_zp = rng.uniform(low=-4000, high=30000, size=(32))
sim_nbiases = sim_biases - sim_zp
convolve(sim_inputs, sim_filters, sim_nbiases, 0x00609664 & ~0x3FFFF, -2, False, True, 0)
