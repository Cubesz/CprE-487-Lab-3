...
inline int32_t MultiplyByQuantizedMultiplier(int32_t x, int32_t multiplier, int shift)
{
    int64_t total_product = (int64_t)x * (int64_t)multiplier;
    if (shift < 0)
        return (int32_t)(total_product << -shift); // Handle left shift

    int64_t nudge = 1LL << (shift - 1);
    int64_t result = (total_product + nudge) >> shift;
