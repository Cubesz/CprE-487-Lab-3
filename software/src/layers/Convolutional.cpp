...
namespace ML
{
    ...
    void ConvolutionalLayer::computeThreaded(const LayerData &dataIn) const
    {
        ...
        computeThreaded(dataIn, 4);
    }

    // Compute the convolution using threads
    // Note: Gemini 3 was consulted on how to make this faster.
    // My own code acheived a decent speedup on its own, but i was curious on
    // if a different mem access pattern that I haven't tried would be faster.
    // Gemini gave feedback regarding using raw pointers, rewriting loops for 
    // optimal compiling using SIMD, and using locality to our advantage. 
    // After implementing these optimizations, a drastic speedup was achieved.
    void ConvolutionalLayer::computeThreaded(const LayerData &dataIn, size_t numThreads) const
    {
        const LayerData &filterData = getWeightData();
        LayerData &outputData = getOutputData();
        const LayerData &biasData = getBiasData();
        ...
        // Get raw pointers to data for maximum speed
        // can cast these because we know the data types
        const float *rawInput = (const float *)dataIn.raw();
        const float *rawFilter = (const float *)filterData.raw();
        const float *rawBias = (const float *)biasData.raw();
        float *rawOutput = (float *)outputData.raw();

        size_t actualThreads = std::min((size_t)numThreads, nOutputRows);
        if (actualThreads < 1)
            actualThreads = 1;

        auto threadWorker = [&](size_t startRow, size_t endRow)
        {
            // Output Rows (Split among threads)
            for (size_t orow = startRow; orow < endRow; ++orow)
            {
                // Output Columns
                for (size_t oc = 0; oc < nOutputColumns; ++oc)
                {
                    // Initialize Output with Bias
                    // This loop is easily vectorized by the compiler (SIMD Store)
                    size_t outputPixelBase = (orow * nOutputColumns * nOutputChannels) + (oc * nOutputChannels);
                    for (size_t och = 0; och < nOutputChannels; ++och)
                    {
                        rawOutput[outputPixelBase + och] = rawBias[och];
                    }
                    ...
                    for (size_t kh = 0; kh < filterHeight; ++kh)
                    {
                        ...
                        for (size_t kw = 0; kw < filterWidth; ++kw)
                        {
                            ...
                            for (size_t ic = 0; ic < nFilterChannels; ++ic)
                            {
                                ...
                                size_t inputPixelRow = orow * stride + kh;
                                size_t inputPixelColumn = oc * stride + kw;
                                size_t inputIndex = (inputPixelRow * inputWidth * nFilterChannels) +
                                                    (inputPixelColumn * nFilterChannels) + ic;

                                // Temporal Locality
                                // load the input pixel once and reuse it for all output channels.
                                float inputVal = rawInput[inputIndex];

                                // Calculate Filter Base Index
                                // points to the start of the output_channel block for this specific weight
                                size_t filterBaseIndex = ((((kh * filterWidth) + kw) * nFilterChannels) + ic) * nOutputChannels;

                                // Output Channels
                                // Spatial Locality & SIMD
                                // The compiler will turn this into AVX/SSE vector instructions.
                                for (size_t och = 0; och < nOutputChannels; ++och)
                                {
                                    rawOutput[outputPixelBase + och] += inputVal * rawFilter[filterBaseIndex + och];
                                }
                            }
                        }
                    }
                    ...
                    for (size_t och = 0; och < nOutputChannels; ++och)
                    {
                        if (rawOutput[outputPixelBase + och] < 0.0f)
                        {
                            rawOutput[outputPixelBase + och] = 0.0f;
                        }
                    }
                }
            }
        ...
        size_t rowsPerThread = nOutputRows / actualThreads;
        size_t remainder = nOutputRows % actualThreads;
        size_t currentStartRow = 0;

        for (size_t i = 0; i < actualThreads; ++i)
        {
            size_t endRow = currentStartRow + rowsPerThread + (i < remainder ? 1 : 0);
            if (currentStartRow < endRow)
            {
                threads.emplace_back(threadWorker, currentStartRow, endRow);
            }
    ...
    {
        computeTiled(dataIn, 2); // Default tile size of 2.
    }

    // Compute the convolution using a tiled approach
    void ConvolutionalLayer::computeTiled(const LayerData &dataIn, size_t tileSize) const
    {
        // Get references to all data and parameters
        const LayerData &filterData = getWeightData();
        LayerData &outputData = getOutputData();
        const LayerData &biasData = getBiasData();
        ...
        const size_t stride = 1;
        const size_t TILE_SIZE = tileSize;

        for (size_t i = 0; i < outputData.getParams().flat_count(); ++i)
        {
            // This is a bit inefficient but simple. bias for the element is at index (i % nOutputChannels)
            outputData.get<fp32>(i) = biasData.get<fp32>(i % nOutputChannels);
        }

        // Tiled Loops for Maximum Cache Reuse
        // The two outermost loops iterate over tiles of the output
        for (size_t orow_base = 0; orow_base < nOutputRows; orow_base += TILE_SIZE)
        {
            for (size_t oc_base = 0; oc_base < nOutputColumns; oc_base += TILE_SIZE)
            {

                // same loops as cache-optimized naive
                // but they now only operate on the data within the current tile.
                for (size_t orow = orow_base; orow < std::min(orow_base + TILE_SIZE, nOutputRows); ++orow)
                {
                    for (size_t oc = oc_base; oc < std::min(oc_base + TILE_SIZE, nOutputColumns); ++oc)
                    {
                        for (size_t kh = 0; kh < filterHeight; ++kh)
                        {
                            for (size_t kw = 0; kw < filterWidth; ++kw)
                            {
                                for (size_t ic = 0; ic < nFilterChannels; ++ic)
                                {
                                    ...
                                    // Fetch input value once (Temporal Locality)
                                    size_t inputPixelIdx = (inputPixelRow * inputWidth * nFilterChannels) + (inputPixelColumn * nFilterChannels) + ic;
                                    fp32 inputValue = dataIn.get<fp32>(inputPixelIdx);

                                    // Innermost loop for sequential filter access (Spatial Locality)
                                    for (size_t och = 0; och < nOutputChannels; ++och)
                                    {
                                        size_t filterPixelIdx = ((((kh * filterWidth) + kw) * nFilterChannels) + ic) * nOutputChannels + och;
                                        fp32 filterValue = filterData.get<fp32>(filterPixelIdx);
