...
namespace ML
{
    ...
    public:
        ...
        template <typename T>
        T get(unsigned int flat_index) const
        {
            // boundsCheck<T>(flat_index);
            return ((T *)data.get())[flat_index];
        }
    ...
    inline void LayerData::loadData(Path filePath)
    {
        ...
        if (filePath.empty())
            throw std::runtime_error("No file path given for required layer data to load from");
    ...
    inline void LayerData::saveData(Path filePath)
    {
        ...
        // Ensure a file path to load data from has been given
        if (filePath.empty())
            throw std::runtime_error("No file path given for required layer data to save to");
...
#ifdef ZEDBOARD
        FIL file;
        if (f_open(&file, params.filePath.c_str(), FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
        { // Open our file on the SD card
#else
    ...
    float LayerData::compare(const LayerData &other) const
    {
        ...
        {
            throw std::runtime_error("Comparison between two LayerData arrays with different element size (and possibly data types) is not advised (" + std::to_string(aParams.elementSize) + " and " + std::to_string(bParams.elementSize) + ")\n");
        }
